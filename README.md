...
The tool needs to parse the header file containing your WinRT entities as if it's compiling it, so _all_ necessary headers have to be included. Make sure you have included all necessary headers. It's suggested that you add `pch.h` to all header files to ease include effort.

#### I made a mistake in the header file, and the library generated an idl file that cannot be compiled/caused C++/WinRT to generate projection that breaks compilation!
...
2. If the mistake caused C++/WinRT to generate any projection that breaks compilation, you need to start over. Copy the old, working idl content from `YourIdl.idl.bak` generated by the library so that idlgen can be bootstrapped. Next, fix your header. Build the project again after you have fixed the header.

If you are in the heat of refactoring a class in a header and do not want the tool to generate idl at all, you can add the header file to project property `IdlGenCppExclude`, either by editing project file or via VS's project property page.
