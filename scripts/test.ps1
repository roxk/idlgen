...
$headers = "BlankPage.h", "PropertyBag.h", "SameViewModel.h", "SomeNamespace\DifferentPathViewModel.h",`
	"SomeNamespace\DifferentPathConsumerViewModel.h", "SomeEnum.h", "SomeFlag.h", "SomeStruct.h", "SomeDelegate.h", "SomeInterface.h",`
	"OutdatedProjection.h", "OutdatedProjectionXaml.xaml.h", "NonWinRtHeader.h"
$rootNamespace = "Root"
...
function gen {
	param([string]$filePath, [switch]$genPch, [switch]$genFakeProjection)
	$LASTEXITCODE = 0
	push-location $testCodeDir
	if ($genPch.IsPresent) {
		$genPchFlags = "--gen-pch"
	}
	if ($genFakeProjection.IsPresent) {
		$genFakeProjectionFlags = "--gen-fake-projection"
	}
	&$idlgen $includes $verboseFlag $filePath --gen $genFakeProjectionFlags $getterTemplatesFlags $propertyTemplatesFlags `
		$pchFlags $pchOutDirFlags $genPchFlags $rootNamespaceFlag $generatedFilesDirFlag | out-host
	pop-location
...
foreach ($header in $headers) {
	gen -filePath $header -genFakeProjection
}

$realProjections = get-childitem "$testDataDir\include" -recurse *.g.h
foreach ($projection in $realProjections) {
	$idlgenHeader = $projection.FullName.Replace(".g.h", ".idlgen.h")
	if (test-path $idlgenHeader) {
		$destination = $projection.FullName.Replace(".g.h", ".g.h.bak")
		if (test-path $destination) {
			remove-item $destination
		}
		move-item $projection -destination $destination 
	}
}
$fakeProjections = get-childitem "$testDataDir\include" -recurse *.idlgen.h
foreach ($projection in $fakeProjections) {
	$desination = $projection.FullName.Replace(".idlgen.h", ".g.h")
	$destinationBak = $projection.FullName.Replace(".idlgen.h", ".g.h.bak")
	if (test-path $destinationBak) {
		move-item $projection -destination $desination
	}
}
...
try {
	...
	$realProjectionBaks = get-childitem "$testDataDir\include" -recurse *.g.h.bak
	foreach ($projection in $realProjectionBaks) {
		$existingHeader = $projection.FullName.Replace(".g.h.bak", ".g.h")
		if (test-path $existingHeader) {
			remove-item $existingHeader
		}
		move-item $projection -destination $existingHeader
	}
}
